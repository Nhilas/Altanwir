...
- `silver.steamReviews` → [`CLUSTER BY (reviewKey)`](../../Fabric/NB_DDL.Notebook/notebook-content.py#L164) (MERGE key; SHA-256 hash distributes evenly for liquid clustering)
- `gold.factReviews` → `gameKey` (read path: `gold.factGameScores` group-by)
