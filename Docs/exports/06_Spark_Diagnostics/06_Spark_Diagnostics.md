...
1. **E00_full_review_load.png** — High-level overview of the entire 77M-row pipeline run
...
|----------|------|-------------|
| E00_full_review_load.png | E00 | End-to-end pipeline run timing for the full 77M review load, showing activity sequence and wall-clock durations across bronze/silver/gold layers with inter-stage wait activities. |
| E01_gold_77m_jobs.png | E01 | Jobs view filtered to gold layer transformations on the 77M run, showing job IDs, task counts, durations, and data volumes for factReviews and related operations. |
...
| E02_bronze_reviews_77m.png | E02 | Bronze write operation (Livy ID a29723ae...) showing wall-clock timing (40m 3s) and four jobs: Job 15 (read, 34m 31s), Job 22 (validation), Job 24 (shuffle), Job 25 (write, 57s) processing 71.1M rows. |
...
| E03_silver_77m_job25_udfs.png | E03 | DAG visualization of nested UDF and CodeGen stages in job 25, Stage 39; displays two distinct ArrowEvalPython stages for VADER sentiment analysis embedded in the Spark execution graph. |
