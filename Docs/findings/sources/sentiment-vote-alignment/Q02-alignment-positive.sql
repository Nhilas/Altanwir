-- Q02 — Positive alignment tail, recognizable IPs, totalReviews >= 50000
-- v2 refactor (Amendment 1): add pctEarlyAccess, genres, themes (collapsed via STRING_AGG); skip echo SELECT for perf
-- Output: G:/Work/Altanwir-scratch/findings-results/Q02-alignment-positive.csv
...
COPY (
  ...
  game_genres AS (
    SELECT gameKey, genreName FROM gold.vw_gameCatalogue
    WHERE genreName IS NOT NULL AND genreName <> 'Unknown'
    GROUP BY gameKey, genreName
  ),
  game_themes AS (
    SELECT gameKey, themeName FROM gold.vw_gameCatalogue
  ...
  LIMIT 10
) TO 'G:/Work/Altanwir-scratch/findings-results/Q02-alignment-positive.csv' (HEADER, DELIMITER ',');
